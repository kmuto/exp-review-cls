...
@<fn>{f2}という脚注を見出し箇所に入れようとするとTeXでは（footnotemark/text化しない限り）エラーになります。見出しにそういうものを入れるべきではない、といえばそれまでですが。見出しにはインラインでの装飾タグが入る可能性があります。
...
説明文に複数の段落を入れることは構文上できないので、@<tt>|@<br>{}|を入れて改行することで代替します。
